...
export const gdriveApi = {
  getConfig: async () => {
    const r = await apiClient.get<ApiEnvelope<GDriveConfig>>("/gdrive/config");
    return unwrap(r.data);
  },
  saveConfig: async (payload: {
    ...
    const r = await apiClient.put<ApiEnvelope<GDriveConfig>>("/gdrive/config", payload);
    return unwrap(r.data);
  },
  test: async () => {
    const r = await apiClient.post<ApiEnvelope<{ ok: boolean; display_name?: string; email?: string; error?: string }>>(
      "/gdrive/test",
    );
    return unwrap(r.data);
  },
  sync: async () => {
    const r = await apiClient.post<ApiEnvelope<{ ok: boolean; fetched: number; upserted: number; error?: string }>>(
      "/gdrive/sync",
    );
    return unwrap(r.data);
  },
  logs: async (limit = 20) => {
    const r = await apiClient.get<ApiEnvelope<SyncLog[]>>("/gdrive/logs", { params: { limit } });
    return unwrap(r.data);
  },
  files: async (limit = 50) => {
    const r = await apiClient.get<ApiEnvelope<GDriveFile[]>>("/gdrive/files", { params: { limit } });
    return unwrap(r.data);
  },
};
